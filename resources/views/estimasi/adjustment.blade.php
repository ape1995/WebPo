{!! Form::open(['route' => 'estimasi.store']) !!}
    ...
    <div class="row">
        ...
        <span id="message_{{ $sort }}" style="display: none"></span>
    </div>
    <script type="text/javascript">
        var sort = "<?php echo $sort; ?>";
        $( "#adjustment_"+sort ).change(function() {
            $('#message_'+"{{ $sort }}").css('display', 'block');
            $('#message_'+"{{ $sort }}").removeClass("badge badge-success");
            $('#message_'+"{{ $sort }}").addClass("badge badge-secondary");
            ...
            $.ajax({
                ...
                {
                    $('#message_'+"{{ $sort }}").removeClass("badge badge-secondary");
                    $('#message_'+"{{ $sort }}").addClass("badge badge-success");
