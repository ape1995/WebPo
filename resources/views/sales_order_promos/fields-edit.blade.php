    ...
    <div class="row">
        ...
        <div class="col-sm-12 mb-1">
            <div class="row">
                ...
                <div class="col-8">
                    <input type="hidden" name="order_id" id="order_id" value="{{ $salesOrderPromo->id }}">
            ...
            <div class="row">
                ...
                <div class="col-8">
                    <select name="order_type" id="order_type" class="form-control select2js" required>
                        <option value="P" {{ $salesOrderPromo->order_type == 'P' ? 'selected' : '' }}>PACKET DISCOUNT</option>
                    </select>
                </div>
            </div>
        </div>
                ...
                <div class="col-8">
                    <select name="customer_id" id="customer_id" class="form-control" readonly>
                        @foreach ($customers as $customer)
                            <option value="{{ $customer->BAccountID }}" {{ $customer->BAccountID == $salesOrderPromo->customer_id ? 'selected' : '' }}>{{ $customer->AcctName }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="col-sm-12 mb-1">
            <div class="row">
                ...
                <div class="col-8">
                    {!! Form::date('delivery_date', $salesOrderPromo->delivery_date, ['class' => 'form-control','id'=>'delivery_date', 'required' => true, 'min' => $minDeliveryDate]) !!}
                </div>
            </div>
        </div>
    </div>
